...
class EnvironTransitionTestCase(unittest.TestCase):
    def test_repr(self):
        envt = EnvironTransition('state1', 'state2', 10,
                                 {'good': False, 'water': 'hydric'})

        test_str = 'start:"state1",\nend:"state2",\nwater:"hydric",\n' + \
                   'good:false,\ndelta_t:10'
...
class ExplicitTestTableTestCase(unittest.TestCase):
    ...
    def get_explicit_test_table(self):
        ...
        test_data_file = os.path.join(test_data_dir,
                                      'test_table_explicit_data.csv')
        return pd.read_csv(test_data_file)

    def setUp(self):
        self.df = self.get_explicit_test_table()
        self.env_trans_set = EnvironTransitionSet(self.df, 'start', 'end',
                                                  'deltat')
    ...
    def test_env_cond_query(self):
        test_envt = EnvironTransition('state1', 'state2', 10,
                                      {'good': False, 'water': 'hydric'})
        ...
        self.assertEqual(self.env_trans_set._get_env_cond_query(test_envt),
                         correct_query)
...
//==============================================================================
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            self.env_trans_set.write_cypher_files(tmpdir)
            test_file = os.path.join(tmpdir, 'succession',
                                     'state1_to_state2_w.cql')
...
class AliasedTestTableTestCase(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.env_cond_aliases = {'cond1': {0: False, 1: True},
    ...
    def test_env_cond_aliases(self):
        self.env_trans_set.apply_environ_condition_aliases(
